...
export default function EmotionManagement() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (axios.isAxiosError(err)) {
        setSubmitError(err.response?.data?.message || t('emotionsDeleteFailed')); // Using this as fallback for generic error too
      } else {
        setSubmitError(t('emotionsDeleteFailed'));
      }
    } finally {
  ...
  const handleDelete = async (id: number) => {
    if (!confirm(t('emotionsConfirmDelete'))) return;
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <h1 className="text-4xl font-black tracking-tight text-slate-900 dark:text-white">
                {t('emotionsTitle').split(' ')[0]} <span className="text-pink-500">{t('emotionsTitle').split(' ')[1] || 'Emotions.'}</span>
              </h1>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          <div className="bg-white/50 dark:bg-slate-900/40 backdrop-blur-sm border border-slate-200 dark:border-slate-800 p-6 rounded-3xl shadow-sm">
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">{t('emotionsTotal')}</p>
          ...
          <div className="bg-white/50 dark:bg-slate-900/40 backdrop-blur-sm border border-slate-200 dark:border-slate-800 p-6 rounded-3xl shadow-sm">
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">{t('emotionsRange')}</p>
                <p className="text-2xl font-black text-slate-900 dark:text-white">{t('emotionsRangeDiverse')}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-slate-950/40 border border-slate-200 dark:border-slate-800 rounded-[2.5rem] overflow-hidden shadow-sm">
          <div className="p-6 md:p-8 border-b border-slate-200 dark:border-slate-800 flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div className="relative flex-1 max-w-md">
              ...
              <input
                type="text"
                placeholder={t('emotionsSearchPlaceholder')}
                value={searchTerm}
          ...
          <div className="overflow-x-auto">
            {loading ? (
              <div className="p-20 flex flex-col items-center justify-center gap-4 text-center">
                ...
                <p className="text-sm font-bold text-slate-400 uppercase tracking-widest font-mono">{t('emotionsLoading')}</p>
              </div>
            ) : error ? (
              <div className="p-20 flex flex-col items-center justify-center gap-4 text-center">
                ...
                <p className="text-slate-900 dark:text-white font-bold">{t('emotionsError')}</p>
                <button onClick={() => reFetch()} className="text-pink-500 font-bold hover:underline">{t('retry')}</button>
              </div>
            ) : filteredEmotions.length === 0 ? (
              <div className="p-20 text-center">
                <p className="text-slate-400 font-medium text-lg italic">{t('emotionsNoResults')}</p>
              </div>
            ) : (
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-slate-50/50 dark:bg-slate-900/50">
                    <th className="px-8 py-4 text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest">{t('emotionsTableState')}</th>
                    <th className="px-8 py-4 text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest">{t('emotionsTableFrequency')}</th>
                    <th className="px-8 py-4 text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest text-right">{t('emotionsTableActions')}</th>
                  </tr>
                </thead>
                  ...
                  {filteredEmotions.map((emo) => (
                    <tr key={emo.id} className="group hover:bg-slate-50/50 dark:hover:bg-slate-900/30 transition-colors">
                      ...
                      <td className="px-8 py-5 text-sm font-medium text-slate-500 font-mono">
                        {t('emotionsOccurrences', { count: emo._count?.failures || 0 })}
                      </td>
          ...
          <div className="relative w-full max-w-md bg-white dark:bg-slate-900 rounded-[2.5rem] shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            <div className="p-8 pb-0 flex items-center justify-between">
              <h3 className="text-2xl font-black text-slate-900 dark:text-white">
                {editingEmotion ? t('emotionsModalUpdate') : t('emotionsModalNew')} <span className="text-pink-500">{t('emotionsModalTitle')}</span>
              </h3>
            ...
            <form onSubmit={handleSubmit} className="p-8 space-y-6">
              <div className="space-y-4">
                <label className="text-xs font-black uppercase tracking-widest text-slate-400">{t('emotionsModalLabel')}</label>
                <div className="relative">
                  ...
                  <input
                    ...
                    placeholder={t('emotionsModalPlaceholder')}
              ...
              <div className="flex gap-4 pt-4">
                ...
                >
                  ...
                  {isSubmitting ? t('emotionsModalProcessing') : editingEmotion ? t('emotionsModalUpdateState') : t('emotionsModalCreate')}
                </button>
              </div>
            </form>
          </div>
