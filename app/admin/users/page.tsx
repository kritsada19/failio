  ...
  const handleUpdateRole = async (id: string, newRole: 'USER' | 'ADMIN') => {
    ...
    try {
      ...
      if (axios.isAxiosError(err)) {
        alert(err.response?.data?.error || t('usersUpdateRoleFailed'));
      } else {
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 pb-6 border-b border-slate-200 dark:border-slate-800">
          <div className="space-y-2">
            ...
            <h1 className="text-4xl font-black tracking-tight text-slate-900 dark:text-white sm:text-5xl">
              {t('usersMainTitle')}<span className="text-blue-500">{t('usersHighlight')}</span>
            </h1>
          ...
          <div className="flex items-center gap-3">
            <div className="px-5 py-3 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 flex flex-col items-end">
              <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{t('usersGrowthTotal')}</span>
              ...
              <div className="flex items-center gap-2 bg-slate-100 dark:bg-slate-900/50 p-1 rounded-2xl">
                {(['ALL', 'USER', 'ADMIN'] as const).map(role => (
                  <button
                    ...
                    className={`px-4 py-2 text-xs font-bold rounded-xl transition-all ${roleFilter === role
                        ? 'bg-white dark:bg-slate-800 text-blue-600 dark:text-blue-400 shadow-sm'
                        : 'text-slate-500 hover:text-slate-900 dark:hover:text-white'
                      }`}
                  >
                    {role === 'ALL' ? t('roleAll') : role === 'USER' ? t('roleUser') : t('roleAdmin')}
                  </button>
                ))}
              </div>
              <button
            ...
            {loading ? (
              <div className="p-24 flex flex-col items-center justify-center gap-6 text-center">
                ...
                <div className="space-y-1">
                  <p className="text-xl font-black text-slate-900 dark:text-white">{t('usersScanning')}</p>
                  <p className="text-sm font-bold text-slate-400 uppercase tracking-widest font-mono">{t('usersScanningDesc')}</p>
                </div>
              </div>
            ) : error ? (
              <div className="p-24 flex flex-col items-center justify-center gap-6 text-center">
                ...
                <div className="space-y-1">
                  <p className="text-xl font-bold text-slate-900 dark:text-white">{t('usersErrorTitle')}</p>
                  <p className="text-sm text-slate-500">{error}</p>
                </div>
                <button onClick={() => reFetch()} className="px-6 py-3 bg-red-600 text-white font-bold rounded-2xl hover:bg-red-700 transition-all active:scale-95">{t('usersReestablishSync')}</button>
              </div>
            ) : users.length === 0 ? (
              <div className="p-24 text-center">
                <p className="text-slate-400 font-bold text-xl uppercase tracking-widest font-mono opacity-50">{t('usersNoResults')}</p>
              </div>
            ) : (
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-slate-50/50 dark:bg-slate-900/50">
                    <th className="px-8 py-4 text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest">{t('usersTableId')}</th>
                    <th className="px-8 py-4 text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest">{t('usersTableAuth')}</th>
                    <th className="px-8 py-4 text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest">{t('usersTableDate')}</th>
                    <th className="px-8 py-4 text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest text-right">{t('usersTableStatus')}</th>
                  </tr>
                </thead>
                  ...
                  {users.map((user) => (
                    <tr key={user.id} className="group hover:bg-slate-50/50 dark:hover:bg-slate-900/30 transition-colors">
                      ...
                      <td className="px-8 py-6">
                        {updatingUserId === user.id ? (
                          <div className="flex items-center gap-2 text-blue-500">
                            ...
                            <span className="text-[10px] font-black uppercase tracking-widest">{t('usersSyncing')}</span>
                          </div>
                        ) : (
                          <div className="flex items-center gap-3">
                            <button
                              onClick={() => handleUpdateRole(user.id, user.role === 'ADMIN' ? 'USER' : 'ADMIN')}
                              className={`flex items-center gap-2 px-3 py-1.5 rounded-xl text-[10px] font-black uppercase tracking-tight transition-all active:scale-95 ${user.role === 'ADMIN'
                      ...
                      <td className="px-8 py-6">
                        <div className="flex flex-col gap-0.5">
                          <span className="text-xs font-bold text-slate-800 dark:text-slate-200 font-mono">
                            {new Date(user.createdAt).toLocaleString(locale === 'th' ? 'th-TH' : 'en-GB', {
                              day: '2-digit',
                              month: 'short',
                              hour: '2-digit',
                              minute: '2-digit'
                      ...
                      <td className="px-8 py-6 text-right">
                        ...
                        <p className="text-[10px] font-bold text-slate-300 dark:text-slate-700 uppercase tracking-widest group-hover:hidden transition-all">{t('usersOperational')}</p>
                      </td>
                    </tr>
                  ))}
                </tbody>
