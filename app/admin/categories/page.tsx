...
export default function CategoryManagement() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingCategory) {
        await axios.put(`/api/admin/category/${editingCategory.id}`, { name: categoryName });
      } else {
        await axios.post('/api/admin/category', { name: categoryName });
      }
      handleCloseModal();
      reFetch();
    } catch (err: unknown) {
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950/50 selection:bg-emerald-100 selection:text-emerald-900 transition-colors duration-300">
              ...
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                  {filteredCategories.map((cat) => (
                    <tr key={cat.id} className="group hover:bg-slate-50/50 dark:hover:bg-slate-900/30 transition-colors">
                      <td className="px-8 py-5">
                        <div className="flex items-center gap-3">
                          <div className="w-8 h-8 rounded-xl bg-emerald-50 dark:bg-emerald-900/20 text-emerald-600 flex items-center justify-center font-bold text-xs">
                      ...
                      <td className="px-8 py-5">
                        <div className="flex items-center justify-end gap-2 translate-x-4 opacity-0 group-hover:opacity-100 group-hover:translate-x-0 transition-all duration-300">
                          <button
                            ...
                            className="p-2 rounded-lg hover:bg-emerald-100 text-slate-400 hover:text-emerald-600 active:scale-90 transition-all"
                          >
                            <Edit2 size={16} />
                          </button>
                          <button
